...
Item {
    ...
    MouseArea {
        ...
        onPositionChanged: {
            if (pressed) {
                var target = initialZoom * Math.pow(
                            5, (initialPos - mouseY) / zoomControl.height)
    ...
    Item {
        ...
        Rectangle {
            id: groove
            x: 0
            y: parent.height * (1.0 - (zoomControl.currentZoom - 1.0)
        ...
        Text {
            ...
            y: Math.min(parent.height - height, Math.max(0,
                                                         groove.y - height / 2))
            font.family: "Centuma"
